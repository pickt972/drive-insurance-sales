...
interface AuthContextType {
  ...
  signOut: () => void;
  users: User[];
  addUser: (username: string, email: string, password: string, role?: 'admin' | 'employee') => Promise<{ success: boolean; error?: string }>;
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addUser = async (username: string, email: string, password: string, role: 'admin' | 'employee' = 'employee'): Promise<{ success: boolean; error?: string }> => {
    try {
      const storedUsers = localStorage.getItem('aloelocation_users');
      const storedPasswords = localStorage.getItem('aloelocation_passwords');

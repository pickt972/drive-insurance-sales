...
// Argumentaires prédéfinis par type d'assurance
const SALES_ARGUMENTS: Record<string, { arguments: string[]; accroches: string[] }> = {
...
// Argumentaire générique pour les types non reconnus
const DEFAULT_ARGUMENTS = {
  arguments: [
    ...
    "Elle simplifie vos démarches en cas de sinistre.",
    "C'est une protection recommandée par les professionnels.",
    "Elle couvre les situations les plus courantes.",
    "Vous bénéficiez d'un accompagnement personnalisé.",
    "Elle s'adapte à vos besoins spécifiques.",
    "C'est une garantie fiable et éprouvée.",
    "Elle vous permet de voyager sereinement.",
    "Les conditions sont transparentes et avantageuses.",
    "Elle protège votre budget contre les aléas.",
    "C'est le choix responsable pour tout conducteur.",
    "Elle offre une couverture claire et complète.",
    "Vous êtes accompagné à chaque étape.",
    "C'est une solution simple et efficace.",
    "Elle répond aux attentes des conducteurs exigeants.",
    "Votre satisfaction est notre priorité."
  ],
  accroches: [
    "Protégez-vous efficacement, simplement.",
    "Votre sécurité, notre engagement.",
    "Roulez tranquille, nous veillons.",
    "La protection qui fait la différence.",
...
export function SalesPitchGenerator({ insuranceType }: SalesPitchGeneratorProps) {
  ...
  const generatePitch = () => {
    ...
    const data = SALES_ARGUMENTS[insuranceType] || DEFAULT_ARGUMENTS;
    
    // Sélectionner 5 arguments aléatoires
    const randomArgs = getRandomItems(data.arguments, 5);
    const randomAccroche = getRandomItems(data.accroches, 1)[0];
